import { validatePolicyEvaluationRequest } from './policy.schema.js';
/**
 ...
 */
function applyProfileRules(profile, request, policy) {
    ...
    if (request.action.url) {
        ...
        if (profile.domains_allow && profile.domains_allow.length > 0 &&
            !profile.domains_allow.some(allowed => domain.includes(allowed))) {
            return {
                decision: 'deny',
    ...
    // Check rate limiting (simplified - would need more sophisticated tracking in real implementation)
    if (profile.budgets && (profile.budgets.rpm || profile.budgets.rph)) {
        // For Day 3, we'll just flag that rate limiting should be checked
        // In a real implementation, you'd check against actual request counts
        return {
            decision: 'allow',
 ...
 */
function shouldRequireApproval(profile, request) {
 ...
 */
export function getActivePolicyVersion(policy) {
    return policy.version;
}
/**
 * Check if policy has specific profile
 */
export function hasProfile(policy, profileName) {
    return profileName in policy.profiles;
}
/**
 * Get profile by name from policy
 */
export function getProfile(policy, profileName) {
